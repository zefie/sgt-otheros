...
static int debug = DEBUG_DETAIL;	// DEBUG_ERROR;
			   ...
			   u8 * value)
{
	...
	if (i == (length + 2)) {
		...
		for (j = 0; j < 10; j++) {
			mdelay(10);
			i = i2c_master_send(client, (u8 *) & i2c_block_transfer,
					    length + 2);
			if (i == (length + 2)) {
				mxt_debug(DEBUG_ERROR,
					  "maXTouch1386 i2c write %d time\n",
					  j + 2);
				return length;
			}
		}
...
static int mxt_read_block_onetime(struct i2c_client *client, u16 addr,
				  u16 length, u8 * value)
{
	struct i2c_adapter *adapter = client->adapter;
	struct i2c_msg msg[2];
...
static int mxt_read_block_wo_addr(struct i2c_client *client, u16 length,
				  u8 * value)
{
	int i;
...
static void CalculateAddr16bits(u8 hbyte_input, u8 lbyte_input, u16 * output,
				u32 val)
{
	u16 temp;
	temp = hbyte_input;
	...
	if (mxt->irq_type == ATMEL_ReadResponseMsg_Noaddress) {
		...
		if (counts == 1) {
			for (i = 0; i <= 1; i++) {	//identify two touch sources two time
				if (mxt_read_block_wo_addr
				    (mxt->client, 8, buffer) < 0) {
					mxt_debug(DEBUG_ERROR,
						  "maXTouch1386: identify CANDO source failed, start to identify Sintek source\n");
					mxt->client->addr = 0x4d;
					if (mxt_read_block_wo_addr
					    (mxt->client, 8, buffer) < 0) {
						if (i == 1) {	//i == 1 means this is second round
							mxt_debug(DEBUG_ERROR,
								  "maXTouch1386: identify two sources failed\n");
							#ifdef USE_SEMA
							up(&mxt->sema);
							#endif
							goto fail;
							...
							mxt_debug(DEBUG_ERROR,
								  "maXTouch1386: identify two sources failed, try again\n");
							mxt->client->addr =
							    0x4c;
							msleep(100);
						}
						...
						mxt_debug(DEBUG_ERROR,
							  "maXTouch1386: It is Sintek source\n");
						break;
					}
				} else {
					mxt_debug(DEBUG_ERROR,
						  "maXTouch1386: It is CANDO source\n");
					break;
				}
			}
		} else {
			if (mxt_read_block_wo_addr(mxt->client, 8, buffer) < 0) {
				mxt_debug(DEBUG_ERROR,
					  "maXTouch1386: mxt_read_block_wo_addr failed\n");
				i2cfail_real = 1;
				#ifdef USE_SEMA
				up(&mxt->sema);
				#endif
		...
		if (buffer[0] == 0x01) {
			if (buffer[1] != 0x00) {
				if ((buffer[1] & 0x08) == 0x08) {
					...
					ConfigError = 0;
					mxt_debug(DEBUG_ERROR,
						  "maXTouch1386: OFL overflow occurs\n");
					#ifdef USE_SEMA
					up(&mxt->sema);
					#endif
					goto still_disable_irq;
		...
		if (mxt_read_block(mxt->client, MsgOBJInf16, 8, buffer) < 0) {
			mxt_debug(DEBUG_ERROR,
				  "maXTouch1386: mxt_read_block failed\n");
			i2cfail_real = 1;
			#ifdef USE_SEMA
			up(&mxt->sema);
			#endif
		...
		i2cfail_real = 0;
		mxt_debug(DEBUG_DETAIL, "print atmel response message\n");
		if (debug == DEBUG_DETAIL) {
			for (i = 0; i <= 7; i++)
				mxt_debug(DEBUG_DETAIL, "buffer is 0x%x\n",
					  buffer[i]);
		...
		i2cfail_real = 0;
		mxt_debug(DEBUG_DETAIL, "print cmd crc message\n");
		if (debug == DEBUG_DETAIL) {
			for (i = 0; i <= 7; i++)
				mxt_debug(DEBUG_DETAIL, "buffer is 0x%x\n",
					  buffer[i]);
		...
		if (mxt_read_block_onetime(mxt->client, MsgOBJInf16, 8, buffer)
		    < 0) {
			i2cfail_esd = 1;
			mxt_debug(DEBUG_ERROR,
				  "maXTouch1386: mxt_read_block failed, try again\n");
			if (mxt_read_block(mxt->client, MsgOBJInf16, 8, buffer)
			    < 0) {
				mxt_debug(DEBUG_ERROR,
					  "maXTouch1386: mxt_read_block fail\n");
				goto fail;
			}
		}
		if (i2cfail_esd == 1) {
		...
		if (buffer[0] >= 2 && buffer[0] <= NUM_FINGERS_SUPPORTED + 1) {
			...
			mxt_debug(DEBUG_DETAIL,
				  "X hbyte is 0x%x xy lbyte is 0x%x\n",
				  buffer[2], buffer[4]);
			...
			mxt_debug(DEBUG_DETAIL,
				  "Y hbyte is 0x%x xy lbyte is 0x%x\n",
				  buffer[3], buffer[4]);
			...
			if ((buffer[1] & 0x20) == 0x20) {
				...
				ContactID = 255;
				mxt_debug(DEBUG_DETAIL,
					  "this point is not touch or release\n");
			}
		} else {
			mxt_debug(DEBUG_DETAIL,
				  "not a member of touch point\n");
		...
		mxt_debug(DEBUG_BASIC,
			  ...
			  ContactID, mxt->PointBuf[ContactID].Status,
			  mxt->PointBuf[ContactID].X,
			  mxt->PointBuf[ContactID].Y);

		/* Send point report to Android */
		if ((mxt->PointBuf[ContactID].Status == 0)
		    || (ContactID <= LastUpdateID) || first_touch) {

			for (i = 0; i < NUM_FINGERS_SUPPORTED; i++) {
				if (mxt->PointBuf[i].Status >= 0) {
					mxt_debug(DEBUG_BASIC,
						  "    Report Point[%d] Update: Status=%d X=%d Y=%d\n",
						  i, mxt->PointBuf[i].Status,
						  mxt->PointBuf[i].X,
						  mxt->PointBuf[i].Y);
...
static ssize_t hbyte_show(struct kobject *kobj, struct kobj_attribute *attr,
			  char *buf)
{
	char *s = buf;
	s += sprintf(s, "0x%x\n", hbyte);
...
static ssize_t hbyte_store(struct kobject *kobj, struct kobj_attribute *attr,
...
static ssize_t lbyte_show(struct kobject *kobj, struct kobj_attribute *attr,
			  char *buf)
{
	char *s = buf;
	s += sprintf(s, "0x%x\n", lbyte);
...
static ssize_t lbyte_store(struct kobject *kobj, struct kobj_attribute *attr,
			   const char *buf, size_t n)
{
	int ibuf;
...
static ssize_t rlen_show(struct kobject *kobj, struct kobj_attribute *attr,
			 char *buf)
{
	char *s = buf;
	s += sprintf(s, "0x%x\n", val);
...
static ssize_t rlen_store(struct kobject *kobj, struct kobj_attribute *attr,
			  const char *buf, size_t n)
{
	rlen = myatoi(buf);
...
static ssize_t debugmsg_show(struct kobject *kobj, struct kobj_attribute *attr,
			     char *buf)
{
	char *s = buf;
	s += sprintf(s, "%d\n", debug);
...
static ssize_t debugmsg_store(struct kobject *kobj, struct kobj_attribute *attr,
			      const char *buf, size_t n)
{
	debug = myatoi(buf);
...
static ssize_t FirmwareVersion_show(struct kobject *kobj,
...
static ssize_t FirmwareVersion_store(struct kobject *kobj,
...
static int ATMEL_CheckOBJTableCRC(struct mxt_data *mxt)
{
	...
	while (1) {
		if ((i + 2) > (MaxTableSize - 3)) {
			...
			Cal_crc =
			    ATMEL_CRCSoft24(Cal_crc, buffer[i], buffer[i + 1]);
		}
		i = i + 2;
	...
	if ((Cal_crc & 0x00ffffff) == InternalCRC) {
		mxt_debug(DEBUG_ERROR,
			  "maXTouch1386: Check OBJ Table CRC is ok....\n");
	} else {
		mxt_debug(DEBUG_ERROR,
			  "maXTouch1386: Check OBJ Table CRC is fail....\n");
		return -1;
	}
	...
	CalculateAddr16bits(MTouchOBJInf[2], MTouchOBJInf[1], &MTouchOBJInf16,
			    0);

	ATMEL_CalOBJ_ID_Addr(TOUCH_KEYARRAY_T15, KeyArrayOBJInf);
	CalculateAddr16bits(KeyArrayOBJInf[2], KeyArrayOBJInf[1],
			    &KeyArrayOBJInf16, 0);
	...
	CalculateAddr16bits(SelfTestOBJInf[2], SelfTestOBJInf[1],
			    &SelfTestOBJInf16, 0);
	...
	CalculateAddr16bits(PalmSUPOBJInf[2], PalmSUPOBJInf[1],
			    &PalmSUPOBJInf16, 0);

	ATMEL_CalOBJ_ID_Addr(SPT_COMCONFIG_T18, ComconfigOBJInf);
	CalculateAddr16bits(ComconfigOBJInf[2], ComconfigOBJInf[1],
			    &ComconfigOBJInf16, 0);

	ATMEL_CalOBJ_ID_Addr(SPT_USERDATA_T38, UserdataOBJInf);
	CalculateAddr16bits(UserdataOBJInf[2], UserdataOBJInf[1],
			    &UserdataOBJInf16, 0);

	ATMEL_CalOBJ_ID_Addr(SPT_DIGITIZER_T43, DigitizerOBJInf);
	CalculateAddr16bits(DigitizerOBJInf[2], DigitizerOBJInf[1],
			    &DigitizerOBJInf16, 0);

	ATMEL_CalOBJ_ID_Addr(SPT_MESSAGECOUNT_T44, MsgcntOBJInf);
	CalculateAddr16bits(MsgcntOBJInf[2], MsgcntOBJInf[1], &MsgcntOBJInf16,
			    0);

	ATMEL_CalOBJ_ID_Addr(PROCI_GRIPSUPPRESSION_T40, GripSUPOBJInf);
	CalculateAddr16bits(GripSUPOBJInf[2], GripSUPOBJInf[1],
			    &GripSUPOBJInf16, 0);

	ATMEL_CalOBJ_ID_Addr(DIAG_DEBUG_T37, DIAGDEBUGOBJInf);
	CalculateAddr16bits(DIAGDEBUGOBJInf[2], DIAGDEBUGOBJInf[1],
			    &DIAGDEBUGOBJInf16, 0);

	if (debug == DEBUG_DETAIL) {
		mxt_debug(DEBUG_DETAIL, "Seperate Table\n");
		mxt_debug(DEBUG_DETAIL,
			  "MsgOBJInf[2] is 0x%x, MsgOBJInf[1] is 0x%x, MsgOBJInf16 is 0x%x\n",
...
static int ATMEL_SendCMDgetCRC(struct mxt_data *mxt)
{
	...
	if (ATMEL_SyncWithThreadToReadMsg(mxt, 0) < 0) {
		mxt_debug(DEBUG_ERROR,
			  "maXTouch1386: ATMEL_SyncWithThreadToReadMsg failed\n");
		return -1;
	}
...
static int ATMEL_WriteConfig(struct mxt_data *mxt)
{
	...
	if (mxt_write_block(mxt->client, ComconfigOBJInf16, 2, ComconfigOBJ) <
	    0)
		return -1;
	...
	if (mxt_write_block(mxt->client, DigitizerOBJInf16, 6, DigitizerOBJ) <
	    0)
		return -1;
...
static int ATMEL_SyncWithThreadToReadMsg(struct mxt_data *mxt, int boot)
{
	...
	if (boot) {
		mxt_debug(DEBUG_DETAIL, "semaphore down\n");
		#ifdef USE_SEMA
		...
		if (error < 0) {
			mxt_debug(DEBUG_ERROR,
				  "maXTouch1386: failed to allocate irq %d\n",
				  mxt->irq);
			mxt_debug(DEBUG_DETAIL, "semaphore up\n");
			#ifdef USE_SEMA
			up(&mxt->sema);
			#endif
			return -1;
...
static int ATMEL_ConfigErrRecovery(struct mxt_data *mxt)
{
	...
	if (ATMEL_WriteConfig(mxt) < 0) {
		mxt_debug(DEBUG_ERROR,
			  "maXTouch1386: ATMEL_WriteConfig failed\n");
		return -1;
	}
...
static int ATMEL_CheckConfig(struct mxt_data *mxt)
{
	...
	if (ConfigChecksumError) {
		do {
			...
			if (ATMEL_Backup(mxt) < 0) {
				mxt_debug(DEBUG_ERROR,
					  "maXTouch1386: ATMEL_Backup failed\n");
				return -1;
			}
			...
			if (ConfigError) {
				mxt_debug(DEBUG_ERROR,
					  "maXTouch1386: ConfigError after Backup\n");
				do {
					if (ATMEL_ConfigErrRecovery(mxt) < 0) {
						mxt_debug(DEBUG_ERROR,
							  "maXTouch1386: ATMEL_ConfigErrRecovery failed\n");
						return -1;
					}
...
static int ATMEL_Initial(struct mxt_data *mxt)
{
	...
	if (ConfigError) {
		...
		if (ATMEL_ReadIDInfo(mxt) < 0) {
			mxt_debug(DEBUG_ERROR,
				  "maXTouch1386: ATMEL_ReadIDInfo failed\n");
			return -1;
		}

		if (ATMEL_CheckOBJTableCRC(mxt) < 0) {
			mxt_debug(DEBUG_ERROR,
				  "maXTouch1386: ATMEL_CheckOBJTableCRC failed\n");
			return -1;
		}
	...
	if (ATMEL_CheckConfig(mxt) < 0) {
		mxt_debug(DEBUG_ERROR,
			  "maXTouch1386: ATMEL_CheckConfig failed\n");
		return -1;
	}
...
static int ATMEL_Issleep(struct mxt_data *mxt)
{
	...
	if (buffer[0] != 0 || buffer[1] != 0) {
		mxt_debug(DEBUG_ERROR,
			  "maXTouch1386: touch panel does not sleep\n");
		return 0;
	}
...
static int ATMEL_IsResume(struct mxt_data *mxt)
{
	...
	if (buffer[0] == 0 || buffer[1] == 0) {
		mxt_debug(DEBUG_ERROR,
			  "maXTouch1386: touch panel does not resume, still sleep\n");
		return 0;
	}
...
static int mxt_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	if (client == NULL) {
		...
		mxt_debug(DEBUG_ERROR,
			  "maXTouch1386: client->adapter == NULL\n");
		return -EINVAL;
		...
		mxt_debug(DEBUG_ERROR,
			  "maXTouch1386: client->adapter->dev == NULL\n");
		return -EINVAL;
	} else if (id == NULL) {
	...
	if (!input) {
		mxt_debug(DEBUG_ERROR,
			  "maXTouch1386: error allocating input device\n");
		error = -ENOMEM;
	...
	if (error < 0) {
		mxt_debug(DEBUG_ERROR,
			  "maXTouch1386: Failed to register input device\n");
...
void mxt_early_suspend(struct early_suspend *h)
{
	...
	if (ATMEL_Deepsleep(data) == 0) {
		mxt_debug(DEBUG_ERROR,
			  "maXTouch1386: ATMEL_Deepsleep OK! ret:%d\n", ret);
	} else {
...
void mxt_late_resume(struct early_suspend *h)
{
	...
	if (ATMEL_Resume(data) == 0) {
		...
		mxt_debug(DEBUG_ERROR,
			  "maXTouch1386: ATMEL_IsResume OK! ret:%d\n", ret);
	} else {
		ret = ATMEL_IsResume(data);
